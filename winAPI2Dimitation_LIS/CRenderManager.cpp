...
void CRenderManager::RenderImage(CD2DImage* img, float dstX, float dstY, float dstW, float dstH)
{
	...
	if (nullptr != img)
	{
		CRenderManager::getInstance()->GetRenderTarget()->DrawBitmap(img->getImage(), imgRect, 1.0f, D2D1_BITMAP_INTERPOLATION_MODE_NEAREST_NEIGHBOR);
	}
}
...
void CRenderManager::RenderFrame(CD2DImage* img, float dstX, float dstY, float dstW, float dstH, float srcX, float srcY, float srcW, float srcH)
{
	...
	if (nullptr != img)
	{
		CRenderManager::getInstance()->GetRenderTarget()->DrawBitmap(img->getImage(), imgRect, 1.0f, D2D1_BITMAP_INTERPOLATION_MODE_NEAREST_NEIGHBOR, srcRect);
	}
}
